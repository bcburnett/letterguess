...
const URI = location.hostname === 'localhost' || location.hostname === '127.0.0.1' ? 'http://localhost/~brian/custom-elements/bcb-elements/bcb-hangman/php/phrases.php' : 'php/phrases.php';
...
export const resetGame = () => async dispatch => {
  let phrase;
  const response = await fetch(URI).catch(() => phrase = {
